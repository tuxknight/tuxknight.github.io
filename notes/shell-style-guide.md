...
| Section                                                      | Contents                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| [背景](#s1-background)                                       | [选择 Shell](#s1.1-which-shell-to-use) - [何时适用 Shell](#s1.2-when-to-use-shell) |
...
可执行文件中，Bash 是唯一被允许使用的 Shell。

可执行文件必须以 `#!/bin/bash` 开头，且尽可能少带参数，而是使用 `set` 给 shell 设置选项。这样可以避免当脚本使用 `bash script_name` 方式执行时参数失效。

将所有可执行的 shell 脚步限制为 *bash* 可以使我们在所有计算机上安装一致的 shell 语言。

除非在编码中被强制要求，否则不允许例外。例如，Solaris SVR4 软件包要求使用纯 Bourne shell 来编写脚本。

<a id="s1.2-when-to-use-shell"></a>

### 何时适用 Shell

Shell 仅适用于小型工具或者简单的封装脚本。

尽管 Shell 不是一种开发语言，但它在 Google 内部被广泛的用来开发各种实用脚本。该指南更多的是对其使用的认可而不是广泛部署的建议。

一些准则：

*  如果你主要是调用其他实用程序，并进行少量的数据操作，那么 shell 是一项可以接受的选择。
*  如果关注性能，请考虑其他的选择。
*  如果你正在编写超过 100 行的脚本，或者使用复杂的控制逻辑，你应该以更结构化的语言重写它。请记住，脚本会不断增长，尽早的重写会避免日后花费更多的时间进行重写。
*   在评估代码的复杂性时，请考虑该代码是否易于其他人维护。
